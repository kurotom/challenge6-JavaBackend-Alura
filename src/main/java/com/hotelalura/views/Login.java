...
//				
	...
	private void Login() {
		...
		pass.append(txtContrasena.getPassword());		
		
		
		if (
				txtUsuario.getText().compareTo("Ingrese su nombre de usuario") != 0 &&
				pass.toString().compareTo("********") != 0
			) {
			
			if (txtUsuario.getText() != "" && pass.toString() != "") {
				
				String usuario = txtUsuario.getText(); 
				String password = pass.toString();
				Funcionario funcionario = new Funcionario(usuario, password);
				
				boolean permitLogin = loginController.login(funcionario);
...
//					MenuUsuario menu = new MenuUsuario();
					...
					JOptionPane.showMessageDialog(this, "Usuario/Contraseña no válidos");
				}
			}
