...
//  Valor - $ 100 por noche
	private Integer precioPorNoche = 100;
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					ReservasView frame = new ReservasView(funcionario);
	 ...
	 * Create the frame.
	 */
	public ReservasView(Funcionario funcionario) {
		super("Reserva");	
		...
		txtFechaE.getCalendarButton().setIcon(new ImageIcon(ReservasView.class.getResource("/com/hotelalura/imagenes/icon-reservas.png")));
...
// Activa el evento, después del usuario seleccionar las fechas se debe calcular el valor de la reserva
				...
				if (txtFechaE.getDate() != null && txtFechaS.getDate()  != null) {
					if (txtFechaE.getDate().compareTo(fechaFormatoAhora) > -1) {
//	Si es mayor a -1, significa que la fecha ingresada es mayor o igual a la fecha actual.
//  Los registros de hospedaje se realizan desde la fecha actual hacia adelante, no hacia atrás.						
						if (
								txtFechaS.getDate().compareTo(txtFechaE.getDate()) == 1
							) {
							
							Long cantidadDias = (txtFechaS.getDate().getTime() - txtFechaE.getDate().getTime()) / 86400000;
							
							if (cantidadDias > 0) {
								setTarifa((double) (cantidadDias * precioPorNoche));
//								System.out.println(getTarifa());
							} else {
								tarifa = (double) precioPorNoche;
//								System.out.println(precioPorNoche);
							}
							
							lblValorSimbolo.setVisible(true);
							txtValor.setVisible(true);
							txtValor.setText(getTarifa().toString());
						}
					} else {
						txtFechaE.setBackground(Color.RED);
					}
		...
		lblValorSimbolo.setBounds(101, 332, 17, 25);
		...
		lblCheckIn.setBounds(68, 136, 202, 14);
		lblCheckIn.setFont(new Font("Roboto Black", Font.PLAIN, 18));
		panel.add(lblCheckIn);
		
		JLabel lblCheckOut = new JLabel("FECHA DE CHECK OUT");
		lblCheckOut.setForeground(SystemColor.textInactiveText);
		lblCheckOut.setBounds(68, 221, 213, 14);
		lblCheckOut.setFont(new Font("Roboto Black", Font.PLAIN, 18));
		panel.add(lblCheckOut);
		
		txtFechaS = new JDateChooser();
		txtFechaS.getCalendarButton().setIcon(new ImageIcon(ReservasView.class.getResource("/com/hotelalura/imagenes/icon-reservas.png")));
...
// Activa el evento, después del usuario seleccionar las fechas se debe calcular el valor de la reserva
				if (txtFechaE.getDate() != null && txtFechaS.getDate()  != null) {
					...
					if (txtFechaS.getDate().compareTo(fechaFormatoAhora) >= 0) {
						if (
								txtFechaS.getDate().compareTo(txtFechaE.getDate()) == 1
							) {
...
//
//	------->     CREA OBJETO 'Reserva' Y LO PASA A VIEW 'RegistroHuesped'.
//					
					if (txtFechaE.getDate().compareTo(fechaFormatoAhora) > -1 || txtFechaS.getDate().compareTo(fechaFormatoAhora) >= 0) {						
						if (txtFechaS.getDate().compareTo(txtFechaE.getDate()) == 1) {
							
							SimpleDateFormat formato = new SimpleDateFormat("yyyy-MM-dd");
							String dateE = formato.format(txtFechaE.getDate());
							String dateS = formato.format(txtFechaS.getDate());

							Reserva unaReserva = new Reserva(dateE, dateS, getTarifa(), txtFormaPago.getSelectedItem().toString());

							RegistroHuesped registro = new RegistroHuesped(unaReserva, funcionario);
						...
						JOptionPane.showMessageDialog(null, "Fechas deben ser mayor o igual a hoy.");
					}
